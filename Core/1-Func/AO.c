...
void set(struct Adaptive_Oscillators* AO, float vw, float va, float vph,
         int order, float dt, float ww, float aa[], float pphh[], int step);
 ...
 *@parameter
  ...
  vw - 频率学习参数
  vph - 相位学习参数
  order - 振荡器池中的振荡器个数，除了没有频率的offset振荡器，因此总共 order+1
 个振荡器 dt - 时间间隔/采样间隔 ww - 基频频率初值 aa[] - 幅值初值 pphh[] -
 相位初值 step - 预测步长
*/
void set(struct Adaptive_Oscillators* AO, float vw, float va, float vph,
         int order, float dt, float ww, float aa[], float pphh[], int step)
 ...
 *@parameter
  ...
  d - 关节角度
*/
void input(struct Adaptive_Oscillators* AO, float y_now, float t_now, int sync,
           float vw_sync);
...
void  Oscillators(struct Adaptive_Oscillators* AO, float e, int sync,
                  float vw_sync);
            ...
            float vw_sync)
{
  ...
  AO->predict =
      curvePredict(AO, AO->step * AO->dt);  // step*2*CONTROL_PERIOD ms
 ...
 * @parameter:
 *      delta_t	-	degree Predict Time interval. It can maxmize
 * independence of this function. AO 		-	Adaptive Oscillators
 * @note:
 *      it is called in input() function
 *
 */
...
/**
 * @brief 单独调用获取相位预测 AO basic phase value (phase[1] which is
 * corresponding to ww's phase) Predict using state
 * @paras
...
int test_phase, test_w, test_output, test_input, test_predict,
    test_phasePredict, test_j;  // jscope
void test_AO(void)
{
  struct Adaptive_Oscillators hip;
...
{
  struct Adaptive_Oscillators hip;
  float                       va, vw, dt, T, aa[3] = {0.0, 15.0, 1.0},
                       pphh[3] = {PI * 0.2, PI / 3.0, PI / 3.0};
  int step;
...
/**
 * @brief
 * 取浮点数的绝对值，由于库中的abs只针对int类型，因此特别创建一个关于浮点数的
 */
                           ...
                           uint8_t node)
{
  ...
  if (stopFlag[node - 1] == 1) {
    ...
    PO_time[node - 1] =
        RESET_TIME - 140;  // convert stop state to walking state.
  ...
  //(Isstable(AO->outputSave)==0)){//&& floatabs(w)>1){
  if (Isequal(d, AO->predictedSaveData[j])) {
  ...
  if (PO_time[node - 1] > RESET_TIME) {
    AO_Init(period[node - 1] * dt, node);  // AO reset 	float w0 = 21*dt;
                                           // // 21*dt
  ...
  if (node == 1) {
    SwitchMonitor("preErr1\t%.1f\tassiveMode1\t%d\t",
                  AO->predict_10steps_save[j] - d, assive);
  }
  if (node == 2) {
    SwitchMonitor("preErr2\t%.1f\tassiveMode2\t%d\t",
                  AO->predict_10steps_save[j] - d, assive);
  }
