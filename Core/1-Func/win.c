 ...
 * @return
 */
uint16_t getCurIndex(WINp winbuffer) { return winbuffer->in; }
 ...
 * @return
 */
void changeLastestValue(WINp winbuffer, ElementType data) { changeValue(winbuffer, 1, data); }
 ...
 * @param	winbuffer - WIN structure pointer
 * @param	index - sequential index  1 代表最新放入的数据  2
 * 代表次新放入的数据  注意 index 不能为0
 * @return
 */
