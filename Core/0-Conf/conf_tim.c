 ...
 *
 * @brief : 普通周期中断模式配置
 * Prescale - timer unit. if it sets 1000, then when timer count is 1000, time
 * passed is 1 second. period - timer time Window > Preferences > C/C++ > Editor
 * > Templates.
 */

void MX_TIM_CounterInterrupt(TIM_TypeDef* TIM, uint32_t Hz,
                             uint32_t period)  // TIM miu_s
{
  if (TIM == TIM2) {
...
void MX_TIM_CaptureInterrupt(TIM_TypeDef* TIM, uint32_t Hz, uint32_t period)
{
  ...
  if (TIM == TIM1) {
    ...
    //		if (HAL_TIM_SlaveConfigSynchro(&htim1, &sSlaveConfig) !=
...
void MX_TIM_SlaveSet(TIM_TypeDef* TIM)
{
  ...
  if (TIM == TIM1) {
    ...
    //默认		sMasterConfig.MasterSlaveMode =
    //TIM_MASTERSLAVEMODE_DISABLE; 		if
...
void MX_TIM_MasterSet(TIM_TypeDef* TIM)
{
  ...
  if (TIM == TIM4) {
    ...
    sMasterConfig.MasterSlaveMode =
        TIM_MASTERSLAVEMODE_ENABLE;  // Enable master/slave mode  sync with
                                     // slave
...
void TIM_PWMSet(TIM_TypeDef* TIM, uint32_t Hz, uint32_t period)
{
  ...
  if (TIM == TIM4) {
    ...
    sConfigOC.OCPolarity =
        TIM_OCPOLARITY_HIGH;  //有效电平,也就是小于比较值的电平，当触发比较后电平反转
    sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
...
void setTimeCount(TIM_TypeDef* TIMx, uint32_t count) { TIMx->CNT = count; }

uint32_t getTimeCount(TIM_TypeDef* TIMx) { return TIMx->CNT; }
