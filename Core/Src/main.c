...
/* 助力值计算 */
float AssisTor = 2.6+0.4; //torqueset 左0.4可克服摩擦 右0.5可克服摩擦 2.62.6+0.4  2.6+0.4  0.9
float RAssistor =2.6+1.0;
...
//	Odrive_Init(CANID_righthip_odriver); /*odrive初始化*/
...
//			printf("Rimu get\r\n");
		...
		if(inc % CONTROL_PERIOD == 0){
			...
			*/
			...
			/* get phase */
			extern float myatan2(float y, float x);
			left_k = AssisTor;//torqueset
			
			if(assive_mode[0] == POMODE){
				phase[0] = APOPhase1(hip1_d, hip1_w);
				phase[0] = phase[0] + PI;
				phaseini[0] = myatan2(hip1_w, hip1_d);
			}
...
//			}
     	...
     	assive_mode[1]= POMODE;
			...
			extern float myatan2(float y, float x);
			/* get phase */
			K = RAssistor ;//torqueset AssisTor
			if(assive_mode[1] == POMODE){
				phase[1] = APOPhase2(hip2_d, hip2_w);
//				printf("\t%.2f", phase[1]);
				phase[1] = -phase[1] + PI;
//				printf("\t%.2f", phase[1]);
				phaseini[1] = -myatan2(hip2_w,hip2_d) + PI ;
			}
				...
				// 防高频率抖动exp
//			extern int stopflag3;
//			SFLAGMonitor("RIGHT %d\t",stopflag3); 
//			if(stopflag3 == 0){
//				K = 0.0;
//				
//			}
...
//			}
		...
		debug_tmp1 = 1000.0*phaseini[0];
		debug_tmp2 = 1000.0*phaseini[1];
